"""Unit tests for canvas_llm — LLM-driven canvas generation."""

from unittest.mock import MagicMock

from onyx.agents.bud_agent.canvas_llm import (
    build_canvas_prompt,
    maybe_generate_canvas,
    should_attempt_canvas,
)
...
class TestBuildCanvasPrompt:
    def test_includes_component_catalog(self) -> None:
        prompt = build_canvas_prompt("Some response text " * 20)
    ...
    def test_includes_response_text(self) -> None:
        ...
        prompt = build_canvas_prompt(response)
        assert "My unique test response" in prompt

    def test_truncates_long_text(self) -> None:
        long_text = "x" * 10000
        prompt = build_canvas_prompt(long_text)
    ...
    def test_includes_none_guidance(self) -> None:
        prompt = build_canvas_prompt("Some text " * 20)
...
class TestMaybeGenerateCanvas:
    def test_short_text_returns_none(self) -> None:
        llm = MagicMock()
        result = maybe_generate_canvas(llm, "Hello!")
    ...
    def test_llm_returns_valid_openui(self) -> None:
        ...
        result = maybe_generate_canvas(llm, text)
    ...
    def test_pre_filter_passes_before_llm_call(self) -> None:
        ...
        result = maybe_generate_canvas(llm, simple_text)
        assert result is None
