...
import { fetchUnifiedCanvas } from "@/lib/workflow/api";
...
import { getWorkflowColor } from "@/lib/workflow/api";
...
export function WorkflowsView() {
  ...
  const handleSelectWorkflow = useCallback((workflow: Workflow) => {
    setFocusedWorkflowId((prev) => (prev === workflow.id ? null : workflow.id));
  }, []);
  ...
  return (
    <div className="flex-1 h-full relative overflow-hidden" data-testid="workflows-view">
      {/* Error state */}
      {error && (
      ...
      {/* Floating sidebar panel — top right, below toggle */}
      ...
      {!isLoading && !error && (!canvasData || canvasData.nodes.length === 0) && (
        <div className="h-full flex flex-col items-center justify-center text-center">
          ...
          <p className="text-sm text-text-02 max-w-sm">
            No repeated tasks yet. As you use the agent for similar tasks,
            canvas items will appear here.
          </p>
        </div>
      )}
    </div>
